...
class Armory_model extends CI_Model
{
	...
	public function getTalents($spec)
	{
		...
		$statements['arkcore'] 		= "SELECT `spell` FROM `character_talent` WHERE `guid` = ? AND `talentGroup` = ? ORDER BY `spell` DESC;";
		$statements['skyfire'] 		= "SELECT `spell` FROM `character_talent` WHERE `guid` = ? AND `talentGroup` = ? ORDER BY `spell` DESC;";
		
		$statements['mangos'] 		= "SELECT `spell` FROM `character_talent` WHERE `guid` = ? AND `talentGroup` = ? ORDER BY `spell` DESC;";
		$statements['mangosr2'] 	= "SELECT `spell` FROM `character_talent` WHERE `guid` = ? AND `talentGroup` = ? ORDER BY `spell` DESC;";
	...
	public function getGlyphs($spec)
	{
		...
		$statements['arkcore'] 		= "SELECT * FROM `character_glyphs` WHERE `guid` = ? AND `talentGroup` = ? LIMIT 1;";
		$statements['skyfire'] 		= "SELECT * FROM `character_glyphs` WHERE `guid` = ? AND `talentGroup` = ? LIMIT 1;";
		
		$statements['mangos'] 		= "SELECT * FROM `character_glyphs` WHERE `guid` = ? AND `talentGroup` = ? LIMIT 1;";
		$statements['mangosr2'] 	= "SELECT * FROM `character_glyphs` WHERE `guid` = ? AND `talentGroup` = ? LIMIT 1;";
	...
	public function getTalentSpecsInfo()
	{
		...
		if ($this->getEmulatorString() == 'arcemu')
		{
			...
			if ($this->storedData)
			{
				return array('talentGroupsCount' => $this->storedData['talentGroupsCount'], 'activeTalentGroup' => $this->storedData['activeTalentGroup']);
			}
			else
				return false;
		}
		...
		$statements['arkcore'] 		= "SELECT `talentGroupsCount`, `activeTalentGroup` FROM `characters` WHERE `guid` = ? LIMIT 1;";
		$statements['skyfire'] 		= "SELECT `talentGroupsCount`, `activeTalentGroup` FROM `characters` WHERE `guid` = ? LIMIT 1;";
		
		$statements['mangos'] 		= "SELECT `talentGroupsCount`, `activeTalentGroup` FROM `characters` WHERE `guid` = ? LIMIT 1;";
		$statements['mangosr2'] 	= "SELECT `talentGroupsCount`, `activeTalentGroup` FROM `characters` WHERE `guid` = ? LIMIT 1;";
	...
	private function getArcemuCharacterDataIfNeeded()
	{
		...
		{
			$query = $this->connection->query("SELECT `glyphs1`, `glyphs2`, `talents1`, `talents2`, `numspecs` AS talentGroupsCount, `currentspec` AS activeTalentGroup, `skills` FROM `characters` WHERE `guid` = ? LIMIT 1;", array($this->id));
